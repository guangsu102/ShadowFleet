...
class XboardRepo:
    ...
    def register_node(self, request: XboardNodeCreateRequest) -> int:
        ...
        parameters = (
            ...
            request.code,
            request.parent_id,
            self._to_json_text(request.group_ids),
            self._to_json_text(request.route_ids),
            node_name,
            request.rate,
            self._to_json_text(request.tags),
            request.host,
            request.port,
            request.server_port,
            self._to_json_text(request.protocol_settings),
            request.show,
            request.sort,
            now,
            now,
            request.rate_time_enable,
            self._to_json_text(request.rate_time_ranges),
        )
