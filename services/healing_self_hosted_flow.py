...
def heal_self_hosted_node(
    *,
    ...
    if not runtime_context.config.cloudflare.enabled:
        raise HealerServiceError("Cloudflare must be enabled for self-hosted healing")
    if node_record.domain_name is None:
        raise HealerServiceError("Self-hosted node domain_name is required for Cloudflare healing")
    ...
    if not record_ids:
        raise HealerServiceError(
            f"No Cloudflare DNS record found for self-hosted node domain={node_record.domain_name}"
        )
    ...
    state_repo.create_event(
        FleetNodeEventCreateRequest(
            ...
            from_status="healing",
            to_status="online",
            message="Self-hosted Cloudflare fallback completed.",
            payload={
    ...
    result = HealResult(
        ...
        duration_ms=duration_ms,
        message="自有机器 Cloudflare 小黄云保底成功",
        correlation_id=runtime_context.correlation_id,
    )
